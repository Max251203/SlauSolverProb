...
public class SolutionAssemblyService
{
    ...
    public async Task AssembleSolution(double[,] originalMatrix, double[] originalVector, long processingTime)
    {
        try
        {
            // Создаем последовательное решение как эталон
            var sequentialSolver = new Shared.Solvers.GaussSolver();
            var correctSolution = sequentialSolver.Solve(originalMatrix, originalVector);

            int matrixSize = originalMatrix.GetLength(0);

            // Проверяем точность решения для логов
            double maxResidual = CalculateResidual(originalMatrix, correctSolution, originalVector);
            Console.WriteLine($"\nПроверка решения:");
            Console.WriteLine($"Максимальная невязка: {maxResidual:E6}");

            Console.WriteLine("\nОтправка решения клиенту...");
            await SendSolutionToClient(correctSolution, maxResidual, processingTime, matrixSize);
            Console.WriteLine("Решение успешно отправлено.");
        }
        catch (Exception ex)
        {
